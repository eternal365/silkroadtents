...
class CMB2_Field {
	...
	public function _set_field_defaults( $args ) {
		...
		$args['options']    = 'group' == $args['type'] ? wp_parse_args( $args['options'], array(
			'add_button'    => __( 'Add Group', 'rolo-slider' ),
			'remove_button' => __( 'Remove Group', 'rolo-slider' ),
		) ) : $args['options'];
		...
		if ( in_array( $args['type'], $option_types, true ) ) {
			...
			$args['show_option_none'] = true === $args['show_option_none'] ? __( 'None', 'rolo-slider' ) : $args['show_option_none'];

			if ( ! $args['show_option_none'] ) {
				$off_by_default = in_array( $args['type'], array( 'select', 'radio', 'radio_inline' ), true );
				$args['show_option_none'] = $off_by_default ? false : __( 'None', 'rolo-slider' );
			}
