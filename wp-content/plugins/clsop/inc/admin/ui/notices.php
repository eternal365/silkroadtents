...
/**
 * This warning is displayed when there is no /opt/cloudlinux-site-optimization-module in the open_basedir.
 *
 * @since 3.13.4
 *
 * @param bool $open_basedir PHP open_basedir value for testing.
 * @return bool|null
 */
function rocket_warning_using_open_basedir( $open_basedir = false ) {
	if ( current_user_can( 'rocket_manage_options' )
		&& rocket_valid_key()
	) {
		if ( ! $open_basedir ) {
		...
		$open_basedir_parts = explode( PATH_SEPARATOR, $open_basedir );
		$found_paths        = array_intersect(
			$open_basedir_parts,
			$paths_to_check
		);
		...
		$message = '
			<strong>AccelerateWP</strong> is currently unable to function properly due to a misconfiguration.
			We recommend reaching out to your hosting provider for assistance.<br><br>
			To resolve this issue, the following value should be appended to the PHP open_basedir setting (be sure to include the colon):
			<pre>:/opt/cloudlinux-site-optimization-module/</pre>
			For more detailed information about the AccelerateWP plugin please refer to our
			<a href="https://docs.cloudlinux.com/user-docs/user-docs-solo-cloudlinux/#acceleratewp" target="_blank">documentation</a>
		';
...
add_action( 'admin_notices', 'rocket_warning_using_open_basedir' );
...
function clsop_warning_message() {
	...
	if ( current_user_can( 'rocket_manage_options' ) ) {
		...
		if ( is_readable( $clsop_notices_file ) ) {
