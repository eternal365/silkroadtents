...
class WC_Deposits_Cart_Manager {
	...
	public function __construct() {
		...
		if( version_compare( WC_VERSION, '3.2', '>=' ) ){
			...
			add_filter( 'woocommerce_cart_tax_totals' , array( $this, 'cart_totals_order_tax_html' ), 10, 2 );
	...
	*/
		...
		foreach ( WC()->cart->get_cart() as $cart_item ) {
			...
			if ( $should_fetch && ( ! empty( $cart_item['is_deposit'] ) && empty( $cart_item['payment_plan'] ) ) ) {
				...
				if ( isset( $cart_item['full_amount'] ) ) {
					if ( ! $include_tax && 'excl' === WC()->cart->tax_display_cart ) {
	...
	public function adjust_cart_total( $total ) {
		...
		//$deferred_tax = $this->calculate_deferred_tax_from_cart();
		//$tax = 'excl' === WC()->cart->tax_display_cart ? $deferred_tax : 0;
	 ...
	 * @return float
	 */
	public function calculate_deferred_tax_from_cart( $subtotal = true ) {
		$deferred_tax = 0;
		foreach ( WC()->cart->get_cart() as $cart_item_key => $cart_item ) {
			...
			if( in_array( $deposit_type, array( 'plan', 'percent', 'fixed') ) ) {
				$full_amount = floatval( $cart_item['full_amount'] );
				$deposit_amount = floatval( $cart_item['deposit_amount'] );
				$quantity = $cart_item['quantity'];
				...
				$deferred_tax += $deferred_proportion * $cart_item['line_tax'];
			} else {
				$deferred_tax += 0;
			}
		}
	...
	private function cart_totals_tax_string( ) {
		...
		if ( ! empty( $cart_tax_totals ) ) {
			$tax = wc_price( WC()->cart->get_taxes_total( true, true ) - $this->calculate_deferred_tax_from_cart() );
			$tax_string_array[] = sprintf( '%s %s', $tax, WC()->countries->tax_or_vat() );
		}
	...
	public function cart_totals_order_tax_html( $taxes ) {
		$deferred_tax = $this->calculate_deferred_tax_from_cart( false );
		if( ! $deferred_tax ) {
		...
		$tax[ $code ] = new stdClass();
		$tax[ $code ]->label = $code;
		$value = wc_price( WC()->cart->get_taxes_total( true, true ) - $deferred_tax );
		$tax[ $code ]->formatted_amount = sprintf( '%s', $value );
		return $tax;
	}
